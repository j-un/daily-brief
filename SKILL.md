...
以下のテンプレートに従ってMarkdownファイルを生成する。

- **出力先ディレクトリ**: `~/rss-digests/`（存在しなければ `mkdir -p` で作成する）
- **ファイル名**: `digest-YYYY-MM-DD.md`
- **フルパス例**: `~/rss-digests/digest-2026-03-29.md`
- 同日に複数回実行した場合は上書きする（最新の結果が残る）
...
- 記事本文の全文取得は基本的に行わない（RSSのtitle・description・linkのみ使用）。ただし Step 3 で必要と判断すれば `web_fetch` で補完してよい
- レポートは日本語で生成する
