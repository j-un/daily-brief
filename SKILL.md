...
- 同日に複数回実行した場合は上書きする（後処理スクリプトも再実行される）
...
### Step 4: 後処理（Markdownリンクのパイプエスケープ）

Step 3 で生成したMarkdownファイルに対して `scripts/postprocess_brief.py` を実行する。
Jekyllがリンクテキスト内の `|` をテーブル構文と誤認してリンクが壊れる問題を防ぐため、
`[text](url)` のテキスト部分にある `|` を `\|` にエスケープする。

```bash
uv run <skill_dir>/scripts/postprocess_brief.py <skill_dir>/docs/brief-YYYY-MM-DD.md
```

`daily-brief.sh` 経由で実行する場合はStep 3の後に自動実行される。
