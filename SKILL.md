...
2. **要約生成**: 残った記事ごとに日本語で **30〜100文字程度** の要約を作成する。
   - 要約は title や summary のコピーではなく、**記事の意義や影響を自分の言葉で簡潔に述べる**こと。
   - summary が定型文や情報不足の場合は、title から推測できる範囲で要約を書く。

   **良い要約の例:**
   - `upstream keep-aliveがデフォルト有効に変更。nginxを本番運用中の場合は要確認。`
   - `CNCFプロジェクトのDaprがAIエージェントオーケストレーション機能を正式リリース。`
   - `PostgreSQLのupsertが予想外の書き込みを発生させるケースのデバッグ事例。`

   **悪い要約の例（これらは禁止）:**
   - `#ばばさん通信ダイジェスト 賛否関わらず話題になった/なりそうなものを共有しています`（← feed の定型文をそのままコピー）
