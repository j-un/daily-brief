...
uv run <skill_dir>/scripts/fetch_feeds.py \
  ...
  --state-file ~/.daily-brief/state.json \
  --hours 24
```
...
- **出力先**: `~/daily-brief/brief-YYYY-MM-DD.md`（ディレクトリは `mkdir -p` で作成）
- 同日に複数回実行した場合は上書きする
...
`~/.daily-brief/state.json` に以下を保存する：
...
> **Note**: GitHub Actions ワークフローでは `./state.json`（リポジトリルート）を使用し、git で状態を引き継ぐ。
