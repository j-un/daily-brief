...
# ///
"""
articles.json を受け取り LLM で関連度判定・カテゴリ分類・注目選定を行い selected.json を出力する。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="LLM で記事を選定し selected.json を生成"
    )
    ...
    if not articles:
        ...
        if args.usage_file:
            ...
            with open(args.usage_file, "w", encoding="utf-8") as f:
                json.dump(
                    {
                        "label": llm_cli.usage_label("select"),
    ...
    print(f"Calling {provider} ({model}) for selection...", file=sys.stderr)
    result_text, usage, cost = llm_cli.call_llm(prompt, role="select")
    print(
        llm_cli.format_usage_line(
            llm_cli.usage_label("select"), usage, cost, provider=provider
        ),
        file=sys.stderr,
    )
